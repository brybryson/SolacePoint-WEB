...
const marineProducts = [
  {
    ...
    desc: 'Cargo Secure provides comprehensive marine cargo insurance that protects goods and merchandise during transit — whether by sea, air, or land. From raw materials to finished products, keep your shipments safe from origin to destination.',
    covers: [
      'Import & export cargo shipments',
      'Domestic inter-island cargo',
      'Air freight & land transit cargo',
  ...
  },
  {
    ...
    desc: 'Hull Secure offers comprehensive marine hull insurance for vessel owners and operators. Protect your ships, boats, and watercraft against physical damage, total loss, and maritime perils.',
    covers: [
      'Commercial cargo vessels & tankers',
      'Passenger ferries & RoRo vessels',
      'Tugboats & barges',
      'Yachts & pleasure craft'
    ]
  },
  {
    title: 'Freight Secure',
    image: 'https://images.unsplash.com/photo-1580674285054-bed31e145f59?q=80&w=1200&auto=format&fit=crop',
    desc: 'Freight Secure is designed for freight forwarders and logistics companies. Protect your liability as a freight forwarder for goods entrusted to your care during the entire transportation process.',
    covers: [
      'Freight forwarder liability coverage',
      'Warehouse-to-warehouse transit',
      'Multi-modal transportation',
      'Consolidation & deconsolidation risks'
    ]
  }
];

const alliedPerils = [
  { name: 'Total Loss (Actual & Constructive)', icon: 'anchor' },
  { name: 'Partial Loss & Damage', icon: 'inventory_2' },
  { name: 'General Average & Salvage', icon: 'directions_boat' },
  { name: 'Perils of the Sea (Storm, Collision)', icon: 'tsunami' },
  { name: 'Fire & Explosion', icon: 'local_fire_department' },
  { name: 'Piracy & Theft', icon: 'warning' },
  { name: 'Jettison & Washing Overboard', icon: 'air' },
  { name: 'Loading & Unloading Damage', icon: 'front_loader' },
  { name: 'Warehouse-to-Warehouse Coverage', icon: 'warehouse' },
  { name: 'War & Strikes (Optional)', icon: 'security' },
  { name: 'Crew Personal Accident', icon: 'personal_injury' },
  { name: 'Third Party Liability', icon: 'handshake' }
];
...
const MarineInsurance = () => {
  ...
  return (
    <div className="font-sans antialiased text-on-surface bg-surface-container-lowest min-h-screen selection:bg-secondary/30">
        ...
        <ScrollReveal direction="up" className="relative z-10">
          <div className="max-w-4xl mx-auto flex flex-col items-center text-center">
            ...
            <p className="font-body-lg text-white/80 max-w-2xl text-lg md:text-xl leading-relaxed mb-10">
              Safeguard your cargo, vessels, and operations by land, air, and sea. Solace Point offers comprehensive marine insurance solutions — from cargo transit to hull protection and freight forwarding.
            </p>
            <button 
      ...
      {/* SECTION 2: Our Marine Insurance Products */}
        ...
        <div className="max-w-container-max mx-auto px-6 md:px-margin-desktop">
          <ScrollReveal direction="up">
            <div className="text-center mb-16 md:mb-20">
              ...
              <h2 className="font-display-lg text-3xl md:text-5xl text-on-surface font-bold">Our Marine Insurance Products</h2>
              <p className="text-on-surface-variant mt-6 text-lg max-w-2xl mx-auto">Comprehensive marine insurance solutions for cargo, vessels, and freight forwarding operations.</p>
            </div>
          </ScrollReveal>

          <div className="flex flex-col gap-24 md:gap-32">
            {marineProducts.map((product, idx) => {
          ...
          <ScrollReveal direction="up">
            <div className="mb-16 md:mb-20 flex flex-col md:flex-row md:items-end justify-between gap-8">
              <div className="max-w-2xl">
                <h2 className="font-display-lg text-3xl md:text-4xl text-on-surface font-bold mb-6">Marine Insurance for Land, Air & Sea</h2>
                <p className="text-on-surface-variant text-lg leading-relaxed">
                  From international shipping to air freight and land transport, we protect every aspect of your cargo and operations.
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 md:gap-6">
              {[
                { name: 'Cargo Ships & Container Vessels', icon: 'directions_boat' },
                { name: 'Air Freight & Cargo Planes', icon: 'flight' },
                { name: 'Land Transport & Trucking', icon: 'local_shipping' },
                { name: 'Passenger Ferries & RoRo', icon: 'directions_boat' },
                { name: 'Tugboats & Barges', icon: 'directions_boat' },
                { name: 'Freight Forwarding Operations', icon: 'inventory_2' }
              ].map((prop, idx) => (
                <div key={idx} className="bg-white px-6 py-8 rounded-2xl border border-outline-variant/30 flex flex-col items-center justify-center text-center hover:border-secondary/30 hover:shadow-md transition-all group">
                  <span className="material-symbols-outlined text-3xl text-secondary/70 mb-4 group-hover:text-secondary group-hover:-translate-y-1 transition-all">{prop.icon}</span>
                  <span className="font-bold text-sm text-primary">{prop.name}</span>
                </div>
              ))}
            </div>
          </ScrollReveal>
        ...
        <div className="max-w-container-max mx-auto px-6 md:px-margin-desktop relative z-10">
          <ScrollReveal direction="up">
            <div className="text-center mb-16 md:mb-20">
              ...
              <h2 className="font-display-lg text-3xl md:text-5xl font-bold mb-6">Comprehensive Marine Protection</h2>
              <p className="text-white/80 text-lg max-w-3xl mx-auto leading-relaxed">
                Our marine insurance policies provide extensive coverage for all transit risks — protecting your cargo, vessels, and business operations by land, air, and sea against loss, damage, and liability.
              </p>
            </div>
          </ScrollReveal>
      ...
      <section className="py-32 md:py-48 bg-primary text-white relative overflow-hidden">
        <div className="absolute inset-0 opacity-20 mix-blend-overlay pointer-events-none">
          <img src="https://images.unsplash.com/photo-1502014822147-1aedfb0676e0?q=80&w=2000&auto=format&fit=crop" alt="Cargo Ship Architecture" className="w-full h-full object-cover" />
        </div>
        ...
        <div className="max-w-container-max mx-auto px-6 md:px-margin-desktop text-center relative z-10">
          <ScrollReveal direction="up">
            <h2 className="font-display-lg text-5xl md:text-6xl lg:text-7xl font-bold mb-8 leading-tight">Protect Your Cargo & <br className="hidden md:block"/> Operations Today</h2>
            <p className="text-white/80 text-lg md:text-xl mb-16 max-w-2xl mx-auto leading-relaxed">
              Get a free, no-obligation quote from multiple top marine insurance providers. Let us find the best coverage for your cargo, vessels, and operations by land, air, and sea.
            </p>
